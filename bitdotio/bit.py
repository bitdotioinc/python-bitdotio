...
@click.option("-q", "--quiet", default=False, help="Suppress all output, but still set an exit code.")
@click.option("-v", "--verbose", default=False, help="Verbose output")
@click.pass_context
def bitio(ctx, key, quiet, verbose):
...
@click.option("-p", "--is_private", default=True, help="Is this repo private or not.")
@click.pass_obj
def create(b, name, description, is_private):
    r = bitdotio.model.repo.Repo(name=name)
...
@click.pass_obj
def retrieve(b, name):
    print(b.retrieve_repo(name))

# bit.py -k your_api_key_here repo destroy -n demo_repo
@repo.command()
@click.option("-n", "--name", required=True, help="The repo name.")
@click.option("-y", "--yes", default=False, is_flag=True, help="Skip the confirmation")
@click.pass_obj
...
def json_data(b, repo_name, table_name, data):
    with data:
        json_str = data.read()
        # this is a way to local json validation
...
@click.pass_obj
def url(b, repo_name, table_name, file_name):
    with data:
        import_file = bitdotio.model.import_file.ImportFile(file_name, table_name, repo_name)
        ingestor_job_details = b.create_import_file(import_file=import_file)
        print(ingestor_job_details)
